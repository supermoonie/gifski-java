...
public class Gifski {
    ...
    public Gifski(GifskiSettings settings) {
        if (settings.getWidth() < 0) {
            throw new IllegalArgumentException("width must be >= 0: " + settings.getWidth());
        }
        if (settings.getHeight() < 0) {
            throw new IllegalArgumentException("height must be >= 0: " + settings.getHeight());
        }
        if (settings.getQuality() < 1 || settings.getQuality() > 100) {
            throw new IllegalArgumentException("quality must be between 1 and 100: " + settings.getQuality());
        }
        this.width = settings.getWidth();
        this.height = settings.getHeight();
        this.quality = settings.getQuality();
        this.once = settings.isOnce();
        this.fast = settings.isFast();
    ...
    private Result result(int code) {
        if (code < 0 || code >= Result.values.length) {
            return Result.OTHER;
        }
    ...
    public void start(final String outputFile) {
        ...
        if (writeThread != null) {
            throw new IllegalStateException("Write thread is already running.");
        }
    ...
    public Result end() {
        if (writeThread == null) {
            throw new IllegalStateException("Write thread is not running.");
        }
        endAddingFrames();
        try {
            writeThread.join();
